...
import random
import math
## Naive bayes src: http://machinelearningmastery.com/naive-bayes-classifier-scratch-python/
...
def separateByClass(df):
	print("Train data with {0} rows of non-objects and {1} rows of objects".format(train[train['class']==0].shape[0], train[train['class']==1].shape[0] ))
...
def separateXandY(df):
	...
	X = df.iloc[:,0:n_cols]
	y = df.iloc[:,n_cols]
	print("Separated features and labels. X = features, y = labels/classifications")
	return (X, y)
...
def main():
	...
	getAccuracy(y_test, getPredictions(train_dict, X_test.values))
